...
[ for network_key, network in local.vnet_json_data : [ for subnet in network.Subnets : { name = subnet.Name, cidr_address = cidrsubnet("10.12.0.0/16", 8, subnet.Priority), vnet = network_key } ] if network_key != "hub-vnet" ]
...
flatten([ for network_key, network in local.vnet_json_data : [ for subnet in network.Subnets : { name = subnet.Name, cidr_address = cidrsubnet("10.12.0.0/16", 8, subnet.Priority), vnet = network_key } ] if network_key != "hub-vnet" ])
